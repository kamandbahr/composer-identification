...
def download_imslp_htmls(args):
    ...
    # paths considered in the webpages! 
    htmls_dir = os.path.join(workspace, 'htmls')
    os.makedirs(htmls_dir, exist_ok=True)

    # download composer page composer base extracting
    ...
    for ch in string.ascii_uppercase[0 : 26]:
        ...
        substring = text[re.search(fr'"{ch}":\[', text).end() :]
    ...
    for n, name in enumerate(names):
        ...
        #data sets got used in the giant midi piano
...
# here is for searching the WikiPedia
        tmp = re.search(r'Detailed biography: <a href="', text)
        if tmp: 
...
def get_music_names_from_imslp(ismlp_path):
    ...
    for link in links:
        ...
        if 'categorypagelink' in link:
            ...
            """
            bgn = re.search(r'title=', link).end()
            link = link[bgn + 1 :]
            fin = re.search(r'>', link).start()
            music_name = link[0 : fin - 1]  
...
def read_csv_to_meta_dict(csv_path):
    """Read csv file to meta_dict using pandas."""
    ...
    while n < min(end_index, len(meta_dict['surname'])):
        ...
        if float(meta_dict['similarity'][n]) > 0.6:
            ...
            if _too_many_requests(error_path):
                ...
                time.sleep(sleep_seconds)
                continue
# if needed converting to mp3 but better for flac which we use in the project for the composer classificaton 
...
def download_youtube_piano_solo(args):
    ...
    os.makedirs(mp3s_dir, exist_ok=True)
    print(mp3s_dir)
    print(os.path.exists(mp3s_dir))  # Should print True
    stdout_path = os.path.join(workspace, '_tmp', 'stdout.txt')
    error_path = os.path.join(workspace, '_tmp', 'error.txt')
    os.makedirs(os.path.dirname(stdout_path), exist_ok=True)
    ...
    while n < min(end_index, len(meta_dict['surname'])):
        ...
        if float(meta_dict['piano_solo_prob'][n]) >= 0.5:
            ...
            youtube_str = 'yt-dlp -f bestaudio -o "{}/{}.%(ext)s" https://www.youtube.com/watch?v={} 1>"{}" 2>"{}"' \
                .format(mp3s_dir, bare_name, meta_dict['youtube_id'][n], stdout_path, error_path)
            ...
            if _too_many_requests(error_path):
                sleep_seconds = 3600
                print('Too many requests! Sleeping for {} s ...'.format(sleep_seconds))
                time.sleep(sleep_seconds)
                continue
